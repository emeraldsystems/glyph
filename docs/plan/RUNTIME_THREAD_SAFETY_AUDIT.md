...
| --- | --- | --- | --- |
| Time: `glyph_time_now`, `glyph_time_monotonic_ns`, `glyph_time_sleep_ms`, `glyph_time_sleep_us`, `glyph_time_to_human_readable` | Only the human-readable function's 20-byte scratch buffer | The buffer is thread-local. Its pointer is valid until the same thread formats another timestamp. Other time APIs use call-local state; `gmtime_r`/`gmtime_s` uses caller-owned state. | The returned text view must be copied into an owned `String` before it can outlive the call boundary. No runtime handle policy is needed. |
